...
local gun1Salvo=Spring.UnitScript.script_weapon_charging_salvo.newBurstWeaponFromWD(unitID,wd1)
--------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------
...
--[[	Hide(rflare)
	...
	StartThread(gun1Salvo.ReloadThread)
end
...
function script.AimWeapon(num, heading, pitch)
	if num == 1 then
		--[=[
		if not gun1Salvo.CanShot() then
			return false
		end]=]
		gun1Salvo.WaitUntilReady()
		Signal(SIG_AIM1)
		SetSignalMask(SIG_AIM1)
		Turn(torso, y_axis, heading, math.rad(1800))
...
function halfJump()
	--Turn(torso, x_axis, 0, math.rad(80))
