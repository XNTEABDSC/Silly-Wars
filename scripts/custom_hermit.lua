...
function script.QueryWeapon(num)
	local wpnnum=unit_weapon_num_to_custom_weapon_num[num]
	if wpnnum==1 then
		return lflare
	elseif wpnnum==2 then
		return rflare
	else
		return base
	end
...
function script.FireWeapon(num)
	local wpn_num=unit_weapon_num_to_custom_weapon_num[num]
	if wpn_num==1 then
		StartThread(recoill)
	elseif wpn_num==2 then
		StartThread(recoilr)
	end
end
