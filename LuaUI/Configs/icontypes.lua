...
if UnitDefs then
  ...
  for _, ud in pairs(UnitDefs) do
    ...
    if cp.icontypes_this then
      local icontype={}
      if cp.icontypes_this_copy_ud then
        local copytask={
          from=UnitDefNames[cp.icontypes_this_copy_ud].iconType,
          to=udname,
        }
        if cp.icontypes_this_copy_scale then
          copytask.scale=tonumber(cp.icontypes_this_copy_scale)
        end
        copytasks[#copytasks+1] = copytask
      elseif cp.icontypes_this_copy then
        local copytask={
          from=cp.icontypes_this_copy,
          to=udname,
        }
        if cp.icontypes_this_copy_scale then
          copytask.scale=tonumber(cp.icontypes_this_copy_scale)
        ...
        if cp.icontypes_this_bitmap then
          icontype.bitmap=cp.icontypes_this_bitmap
        end
        if cp.icontypes_this_size then
          icontype.size=tonumber( cp.icontypes_this_size )
        end
        if cp.icontypes_this_radiusadjust then
          icontype.radiusadjust=tonumber(cp.icontypes_this_radiusadjust)
        end
        if cp.icontypes_this_distance then
          icontype.distance=tonumber(cp.icontypes_this_distance)
        end
        icontypes[udname]=icontype
      end
      
    end
    if cp.icontypes_custom then
      for ictname, ict in pairs(utils.justeval(cp.icontypes_custom)) do
        icontypes[ictname]=ict
      end
    end
  end
  local failed_tasks = utils.loop_until_finish_all(copytasks,function (task)
    Spring.Echo("icontype task: from " .. task.from .." to " .. task.to)
    if icontypes[task.from] then
      icontypes[task.to]=Spring.Utilities.CopyTable( icontypes[task.from] )
      if task.scale then
        icontypes[task.to].size=(icontypes[task.to].size or 1)*task.scale
      end
      return true
    else
      return false
  ...
  if failed_tasks then
    for _, task in pairs(failed_tasks) do
      Spring.Echo("failed icontype task: from " .. task.from .." to " .. task.to)
    end
  end
elseif utils.gamedata_UnitDefs then
  local copytasks={}
  for _, ud in pairs(utils.gamedata_UnitDefs) do
    local udname=ud.name
    local cp=ud.customparams
    if cp.icontypes_this then
      local icontype={}
      if cp.icontypes_this_copy_ud then
        local copytask={
          from=utils.gamedata_UnitDefs[cp.icontypes_this_copy_ud].iconType,
          to=udname,
        }
        if cp.icontypes_this_copy_scale then
          copytask.scale=tonumber(cp.icontypes_this_copy_scale)
