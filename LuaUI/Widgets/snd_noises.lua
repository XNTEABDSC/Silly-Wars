...
local function playSound(filename, ...)
	...
	if (VFS.FileExists(path)) then
		...
		if VFS.FileExists(SOUND_BASE_DIR .. filename .. ".WAV") then
			PlaySoundFile(filename, ...)
		else
			Spring.Echo("<snd_noises.lua>: Error file "..path.." doesn't exist. file " .. SOUND_BASE_DIR ..filename .. " doesn't exist.")
		end
